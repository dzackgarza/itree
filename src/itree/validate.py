...
DIAGNOSTIC_CATALOG = {
    ...
    "W040": {
        ...
        "severity": "warning",
        "meaning": (
            "The tree defines traversal. The GitHub milestone defines release/time grouping. "
            "They should normally agree so Projects and milestone progress views remain useful."
        ),
        "remediation": [
...
def generate_doctor_report(dag: RepoDag, root_flag: str | None = None) -> DoctorReport:
    ...
    if root_ref is not None:
        ...
        for wu in sorted(work_unit_nodes, key=lambda w: w.issue.number):
            child_work_units = [
                d for d in wu.descendants()
                if d.issue.is_open and not is_grouping_issue(d.issue.title)
            ]
            if len(child_work_units) > 10:
                oversized_wu_findings.append(
                    f"work-unit branch #{wu.issue.number} contains {len(child_work_units)} child work-unit issues"
                )
        ...
        missing_ac_findings = []
        open_work_units = [
            n for n in work_unit_nodes
            if not any(desc.issue.is_open for desc in n.descendants())
        ]
