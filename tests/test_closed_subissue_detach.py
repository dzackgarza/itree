...
def test_detach_removes_a_closed_direct_subissue_and_restores_the_fixture(request: pytest.FixtureRequest) -> None:
    ...
    def cleanup() -> None:
        ...
        current_child = api.get_issue(child.number)
        if current_child.is_open:
...
def test_absorb_detaches_a_closed_direct_subissue_and_restores_target_body(request: pytest.FixtureRequest) -> None:
